...
import type { ChatwootCredentials } from "../lib/chatwoot-api";
import {
...
const configSchema = z.object({
  ...
  handoffTeamId: z.number().nullable(),
  handoffAssigneeId: z.number().nullable(),
  retrievalTopK: z.number(),
  retrievalFinalK: z.number(),
});
...
function isHumanHandoffConfigured(cfg: z.infer<typeof configSchema>): boolean {
  return (
    cfg.enableHandoff &&
    Boolean(cfg.handoffAssigneeId || cfg.handoffTeamId)
  );
}

function getChatwootCredentials(cfg: z.infer<typeof configSchema>): ChatwootCredentials {
  return {
...
function buildSafeFallbackReply(
  ...
  const cfg = inputData.config as unknown as AppConfig;

  if (inputData.handoffRequested) {
    return isHumanHandoffConfigured(inputData.config)
      ? buildHandoffConfirmationReply()
      : buildUnavailableHandoffReply(cfg, inputData.messageContent);
  }

  switch (inputData.route.intent) {
    case "smalltalk_or_greeting":
      return buildGreetingReply(cfg);
    case "out_of_scope":
      return buildOutOfScopeReply(cfg);
    case "sensitive":
      return buildSensitiveReply(cfg);
    case "needs_clarification":
      return buildClarificationReply(inputData.judgement.missingInfo);
    case "handoff_request":
      return buildUnavailableHandoffReply(cfg, inputData.messageContent);
    case "local_factual":
    default:
...
const validateWebhook = createStep({
  id: "validate-webhook",
  description:
    "Validates the incoming Chatwoot webhook, extracts relevant data, and loads config from DB",
...
const retrieveContext = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    const evidence = await retrieveLocalEvidence({
      ...
      topK: inputData.config.retrievalTopK,
      finalK: inputData.config.retrievalFinalK,
    });
...
const composeCitizenReply = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    if (handoffRequested) {
      return {
        ...inputData,
        draftReply: isHumanHandoffConfigured(inputData.config)
          ? buildHandoffConfirmationReply()
          : buildUnavailableHandoffReply(cfg, inputData.messageContent),
        shouldSend: true,
        handoffRequested,
    ...
    if (inputData.route.intent === "smalltalk_or_greeting") {
      return {
        ...inputData,
        draftReply: buildGreetingReply(cfg),
        shouldSend: true,
        handoffRequested: false,
      };
    }
    ...
    if (inputData.route.intent === "out_of_scope") {
      return {
        ...inputData,
        draftReply: buildOutOfScopeReply(cfg),
        shouldSend: true,
        handoffRequested: false,
      };
    }
    ...
    if (
      ...
      return {
        ...inputData,
        draftReply: buildSensitiveReply(cfg),
        shouldSend: true,
        handoffRequested: false,
      };
    }
...
const sendReply = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    const credentials = getChatwootCredentials(inputData.config);
    const cfg = inputData.config as unknown as AppConfig;
    let outboundReply = inputData.outboundReply;
    let handoffPerformed = false;

    if (inputData.handoffRequested) {
      if (isHumanHandoffConfigured(inputData.config)) {
        try {
          await assignChatwootConversation(credentials, {
            accountId: inputData.accountId,
            conversationId: inputData.conversationId,
            assigneeId: inputData.config.handoffAssigneeId ?? undefined,
            teamId: inputData.config.handoffTeamId ?? undefined,
          });

          handoffPerformed = true;

          await sendChatwootMessage(credentials, {
            accountId: inputData.accountId,
            conversationId: inputData.conversationId,
          ...
          outboundReply = buildUnavailableHandoffReply(cfg, inputData.messageContent);
        }
      } else {
        outboundReply = buildUnavailableHandoffReply(cfg, inputData.messageContent);
      }
    ...
    const data = await sendChatwootMessage(credentials, {
      accountId: inputData.accountId,
      conversationId: inputData.conversationId,
